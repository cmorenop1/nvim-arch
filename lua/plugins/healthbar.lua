  ...
  config = function()
    ...
    local function open_health()
      ...
      vim.api.nvim_buf_call(health_buf, function()
        ...
        -- Capture the channel ID returned by termopen

        health_chan = vim.fn.termopen("bash ~/scripts/healthbar.sh", {
    ...
    local function reset_health()
      ...
      if health_chan then
        ...
        -- Send 'r' to the terminal channel to trigger the bash script's reset

        vim.api.nvim_chan_send(health_chan, "r")

      else

        -- If it's not open, just open it fresh
